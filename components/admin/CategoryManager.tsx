...
import { Plus, Settings2, Trash, Tag, BookOpen, Package } from 'lucide-react';
import { Category } from '../../types';

interface CategoryManagerProps {
    categories: Category[];
    handleAdd: (type: 'category', initialData?: any) => void;
...
export const CategoryManager: React.FC<CategoryManagerProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'BLOG' | 'PACKAGE'>('BLOG');

    const filteredCategories = (categories || []).filter(cat => 
        activeTab === 'BLOG' 
            ? (!cat.type || cat.type === 'BLOG') 
            : cat.type === 'PACKAGE'
    );
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 font-medium text-sm mt-1">
                        Video, haber ve paket kategorilerini yönetin.
                    </p>
                </div>
                <button
                    onClick={() => handleAdd('category', { type: activeTab })}
            ...
            <div className="flex space-x-2 p-1 bg-slate-100 rounded-xl w-fit">
                <button
                    ...
                    className={`flex items-center space-x-2 px-6 py-2.5 rounded-lg text-sm font-black transition-all ${
                        activeTab === 'BLOG'
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <span className="text-xs font-medium text-slate-400 capitalize flex items-center gap-1">
                                        <div className={`w-1.5 h-1.5 rounded-full ${cat.type === 'PACKAGE' ? 'bg-purple-500' : 'bg-brand-blue'}`}></div>
                                        {cat.type === 'PACKAGE' ? 'Paket' : 'Haber'}
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            {filteredCategories.length === 0 && (
                <div className="text-center py-20">
                    ...
                    <p className="text-sm text-slate-400 mb-6">Bu grupta yeni kategori ekleyerek başlayın</p>
                    <button
                        onClick={() => handleAdd('category', { type: activeTab })}
